...
async function crypto_hash(out: Uint8Array, m: Uint8Array, n: number) {
  const hash = await sha512(m.subarray(0, n));
...
async function crypto_sign_keypair(pk: Uint8Array, sk: Uint8Array) {
  var d = new Uint8Array(64);
  var p = [gf(), gf(), gf(), gf()];
  var i;

  await crypto_hash(d, sk, 32);
  d[0] &= 248;
  d[31] &= 127;
  d[31] |= 64;
...
async function crypto_sign(sm: Uint8Array, m: Uint8Array, n: number, sk: Uint8Array) {
  var d = new Uint8Array(64),
    h = new Uint8Array(64),
    r = new Uint8Array(64);
  var i,
  ...
  await crypto_hash(r, sm.subarray(32), n + 32);
  reduce(r);
  scalarbase(p, r);
  pack(sm, p);

  for (i = 32; i < 64; i++) sm[i] = sk[i];
  await crypto_hash(h, sm, n + 64);
  reduce(h);
...
async function crypto_sign_open(m: Uint8Array, sm: Uint8Array, n: number, pk: Uint8Array) {
  var i, mlen;
...
async function sign(msg: Uint8Array, secretKey: Uint8Array) {
...
export async function sign_detached(msg: Uint8Array, secretKey: Uint8Array) {
  var signedMsg = await sign(msg, secretKey);
  var sig = new Uint8Array(crypto_sign_BYTES);
  for (var i = 0; i < sig.length; i++) sig[i] = signedMsg[i];
  return sig;
}

export async function sign_detached_verify(msg: Uint8Array, sig: Uint8Array, publicKey: Uint8Array) {
  ...
  const result = await crypto_sign_open(m, sm, sm.length, publicKey);
  return result >= 0;
}

export async function sign_keyPair_fromSeed(seed: Uint8Array) {
...
export async function hash(msg: Uint8Array) {
  var h = new Uint8Array(crypto_hash_BYTES);
  await crypto_hash(h, msg, msg.length);
  return h;
}
